...
export class DataGenerateService implements IDataGenerateService {
  ...
  async adjustData() {
    // const res = await db.query("Select user_id, created_at from users where created_at >= now() - interval '24 hours'");
    const res = await db.query("Select user_id, created_at, last_session from users where last_session > created_at + interval '24 hours'");
    console.log(res[0]);
  }

  static runService() {
    // new DataGenerateService([anonymousSession]);
